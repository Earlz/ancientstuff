...
void printf(char*);
byte abswriteN(byte,byte,word,char*);
byte absreadN(byte,byte,word,char*);
...
void segsave(word seg,word sadr){
	dword counter=0;
	word offset;
	byte buffer[512];
	word cnt=0;
	...
	while (cnt<=512){
		peekb(seg,counter,&buffer[cnt]);
		cnt++;
	}
	abswriteN(0,1,sadr+c,&buffer);
	c++;
	cnt=0;
	counter=counter+512;
}
...
void segrestore(word seg,word sadr){
	dword counter=0;
	word offset;
	byte buffer[512];
	word cnt=0;
	word c=0;
	while (counter<0xffff){
absreadN(0,1,sadr+c,&buffer);
	while (cnt<=512){
		pokeb(seg,counter,buffer[cnt]);
		cnt++;
	}
	c++;
	cnt=0;
