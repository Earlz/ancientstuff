...
#define VERSION 	"1.0"
#define ALIGNMENT	4ul				//< This is the byte alignment that memory must be allocated on. IMPORTANT for GTK and other weird stuff.
...
#define ALIGN( ptr )													\
		...
		{																\
			unsigned int diff;											\
			ptr = (void*)((unsigned int)ptr + ALIGN_INFO);				\
			diff = (unsigned int)ptr & (ALIGNMENT-1);					\
			if ( diff != 0 )											\
			{															\
				diff = ALIGNMENT - diff;								\
				ptr = (void*)((unsigned int)ptr + diff);				\
			}															\
			*((ALIGN_TYPE*)((unsigned int)ptr - ALIGN_INFO)) = 			\
				diff + ALIGN_INFO;										\
		}															
...
#define UNALIGN( ptr )													\
		if ( ALIGNMENT > 1 )											\
		{																\
			unsigned int diff = *((ALIGN_TYPE*)((unsigned int)ptr - ALIGN_INFO));	\
...
#if defined DEBUGM || defined INFO
#include <stdio.h>
#include <stdlib.h>
...
void *PREFIX(malloc)(size_t req_size)
{
	...
	if ( ALIGNMENT > 1 )
	{
		size += ALIGNMENT + ALIGN_INFO;
				...
				// to save space but NO... GTK is daft.
	...
	while ( maj != NULL )
	{
		...
		while ( min != NULL )
		{
				...
				if ( min->next == NULL )
				{
					...
					diff = (unsigned int)(maj) + maj->size;
